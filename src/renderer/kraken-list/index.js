...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('back-button-kraken-list').addEventListener('click', () => {
  ...
  if (connectSensorsBtn) {
    connectSensorsBtn.addEventListener('click', async () => {
      const selectedSensors = Array.from(document.querySelectorAll('.sensor-checkbox:checked')).map(
        checkbox => checkbox.dataset.sensorId
      );
  ...
  if (searchInput) {
    searchInput.addEventListener('input', () => {
      ...
      let filtered = discoveredDevices.filter(device => {
        return device.id.toLowerCase().includes(query) || device.name.toLowerCase().includes(query);
      });
...
window.electronAPI.onDeviceUpdated(device => {
  ...
  if (index !== -1) {
    ...
    if (signalStrength) {
      signalStrength.innerText = `${signalData.strength} (${device.rssi})`;
    }
...
window.electronAPI.onDeviceConnectionSuccess(data => {
  ...
  console.log(
    `Successfully connected device ${currentIndex}/${totalCount}. Total connected: ${connectedCount}`
  );
});
