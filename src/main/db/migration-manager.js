...
export class MigrationManager {
  ...
  getCurrentVersion() {
    try {
      const result = this.db.prepare(`SELECT MAX(version) as version FROM ${this.migrationsTable}`).get();
  ...
  validateMigration(migration) {
    ...
    if ((typeof migration.up !== 'string' || migration.up.trim() === '') && (!Array.isArray(migration.up) || migration.up.length === 0)) {
  ...
  applyMigration(migration) {
    ...
    try {
      ...
      if (Array.isArray(migration.up)) {
        ...
        this.executeMultipleStatements(migration.up);
      } else if (migration.up.includes('ALTER TABLE')) {
  ...
  /**
   * Execute multiple SQL statements from an array
   */
